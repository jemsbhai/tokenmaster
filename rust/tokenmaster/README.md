...
- A bundled model registry (15 models with dated, cited pricing), alias and
  ...
  so the core does no filesystem or network access at run time. GPT-5.6 Sol,
  Terra, and Luna include Standard short- and long-context tiers.
...
provenance says so), and async event delivery.

## Design

The core depends on serde and serde_json only and never touches the network.
The repository's separately invoked Python `tokenmaster-models` maintainer
